...
def honest_arch():
    ...
    arch = Archivist("https://app.rkvst-poc.io", (client_id, client_secret), max_time=300)
...
def create_asset(arch, displayname, description, displaytype, version, id, hash, attachments, doc_attachments):
    attrs = {
...
def create_c2docasset(arch, image):
    ...
    return create_asset(arch, "C2PA Image - Vinyl", "C2PA Image - Vinyl", "C2PA", "1.0.0", serial_num, doc_hash, attachments, doc_attachments)
...
def create_manifest(arch, id, image, definition):
    ...
    cmd = ['c2patool', 'sample/' + image, '-m', 'sample/' + definition, '-f', '-o', 'sample/signed_' + image]
...
def create_ingredient(arch, id, image):
    ...
    cmd = ['c2patool', 'sample/' + image, '--ingredient', '--force', '--output', './ingredient']
...
def create_parent(arch, id, image, definition):
    ...
    cmd = ['c2patool', 'sample/' + image, '-m', 'sample/' + definition, '-p', './ingredient', '-f', '-o', 'sample/signed_' + image]
    subprocess.run(cmd, shell=False)
...
def main():
    ...
    create_manifest(arch, asset["attributes"]["id"], "pexels-miguel-á-padriñán-5764283.jpg", "definition.json")
    create_publish(arch, asset["attributes"]["id"], "1.1.M", "signed_pexels-miguel-á-padriñán-5764283.jpg", "Published Asset-Embedded Manifest", "Honest Abe")
    create_ingredient(arch, asset["attributes"]["id"], "signed_pexels-miguel-á-padriñán-5764283.jpg")
    create_publish(arch, asset["attributes"]["id"], "2.0.0", "pexels-miguel-á-padriñán-5764284.jpg","Published Related Image", "Honest Abe")
    create_parent(arch, asset["attributes"]["id"],"pexels-miguel-á-padriñán-5764284.jpg", "4definition.json")
    create_publish(arch, asset["attributes"]["id"], "2.1.PM", "signed_pexels-miguel-á-padriñán-5764284.jpg", "Published Parent Manifest", "Honest Abe")
    create_details(arch, asset["attributes"]["id"], "signed_pexels-miguel-á-padriñán-5764284.jpg")

    time.sleep(2)

    evil_asset = create_c2docasset(bad_arch, "pexels-miguel-á-padriñán-5764283.jpg")
    create_manifest(bad_arch, evil_asset["attributes"]["id"], "pexels-miguel-á-padriñán-5764283.jpg", "bad_definition.json")
    create_publish(bad_arch, evil_asset["attributes"]["id"], "1.1.M", "signed_pexels-miguel-á-padriñán-5764283.jpg", "Published Asset-Embedded Manifest", "Evil Eddie")
    create_ingredient(bad_arch, evil_asset["attributes"]["id"], "signed_pexels-miguel-á-padriñán-5764283.jpg")
    create_publish(bad_arch, evil_asset["attributes"]["id"], "2.0.0", "pexels-miguel-á-padriñán-5764284.jpg","Published Related Image", "Evil Eddie")
    create_parent(bad_arch, evil_asset["attributes"]["id"],"pexels-miguel-á-padriñán-5764284.jpg", "4bad_definition.json")
    create_publish(bad_arch, evil_asset["attributes"]["id"], "2.1.PM", "signed_pexels-miguel-á-padriñán-5764284.jpg", "Published Parent Manifest", "Evil Eddie")
    create_details(bad_arch, evil_asset["attributes"]["id"], "signed_pexels-miguel-á-padriñán-5764284.jpg")
